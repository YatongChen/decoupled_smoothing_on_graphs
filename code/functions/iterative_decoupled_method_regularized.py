def iterative_decoupled_method_regularized(t, G, gender_y_update, theta0, train_index, test_index):
...
#                theta[j] = np.sum(theta0[ii] for ii in two_hop_neighbor)/len(two_hop_neighbor)
                lambda_j = (len(two_hop_labeled_neighbor)/len(two_hop_neighbor)/ratio)**((t-1))
                 #lambda_j = (0.85)**(t-1)
