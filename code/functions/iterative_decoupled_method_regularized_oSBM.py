def iterative_decoupled_method_regularized_oSBM(t, G, gender_y_update, theta0, train_index, test_index, bench_mark):
    ...
    if t ==1:
        ...
        # if it is the first iteration, which is reduced to the 2-hop method
            ...
            if len(two_hop_neighbor) != 0:
                test_index_non_na.append(j)
                #lambda_j = (t-1)*(len(two_hop_labeled_neighbor)/len(two_hop_neighbor))**(t-1)
                #lambda_j = (0.85)**(t-1)
                #lambda_j = np.exp(-len(two_hop_labeled_neighbor)/len(two_hop_neighbor))*(len(two_hop_labeled_neighbor)/len(two_hop_neighbor))**(t-1)
                lambda_j = (len(two_hop_labeled_neighbor)/len(two_hop_neighbor))**(t-1)
